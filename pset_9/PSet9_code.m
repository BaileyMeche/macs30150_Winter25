...
for s=1:TT+1
    ...
    mp_min = y + tau; % CIA constraint (m' = y + tau)
    mp_max = 2.5;  % Maximum money level (optional)
...
%% Equilibrium using the bisection method
...
m_min = 0.5;  % Lower bound for money supply
m_max = 5.0;  % Upper bound for money supply
tol = 1e-6;   % Tolerance for convergence
max_iter = 100; % Maximum iterations for bisection

% Bisection algorithm to find equilibrium money supply
iter = 0;
diff = tol + 1;  % Initialize difference above tolerance

disp('Starting bisection method to find equilibrium money supply...');
...
while (diff > tol && iter < max_iter)
    ...
    % Calculate excess demand at m_min and m_mid
    excess_min = compute_excess_demand(m_min, gamma, y, beta, sigma, theta_1, theta_2, QQ);
    excess_mid = compute_excess_demand(m_mid, gamma, y, beta, sigma, theta_1, theta_2, QQ);
    
    % Check if the root is in the left or right subinterval
    if excess_min * excess_mid <= 0
        m_max = m_mid;
    else
        m_min = m_mid;
    end
    
    % Compute new difference for convergence check
    diff = m_max - m_min;
    
    fprintf('Iteration %d: Money supply = %.6f, Excess demand = %.6f, Interval width = %.6f\n', ...
        iter, m_mid, excess_mid, diff);
end

% Final equilibrium money supply
m_equilibrium = (m_min + m_max) / 2;
fprintf('\nEquilibrium money supply found: %.6f after %d iterations\n', m_equilibrium, iter);

% Calculate tau at equilibrium
tau_equilibrium = m_equilibrium * gamma / (1 + gamma);
fprintf('Equilibrium tau: %.6f\n', tau_equilibrium);

% Now calculate policy functions and distributions at equilibrium
[g_pol_1, g_pol_2, FO, dist_mgrid, c_pol_1, c_pol_2, mgrid] = compute_policy_and_distribution(m_equilibrium, gamma, y, beta, sigma, theta_1, theta_2, QQ);
...
% Plotting consumption function
figure;
plot(dist_mgrid, c_pol_1, 'r'); hold on;
plot(dist_mgrid, c_pol_2, 'b');
xlabel('Money');
ylabel('Consumption');
legend('Low theta', 'High theta');
title('Policy function: Consumption');

% Define the function to compute excess demand (money demand - money supply)
function excess = compute_excess_demand(money_supply, gamma, y, beta, sigma, theta_1, theta_2, QQ)
    % First get policy functions and distributions
    [~, ~, FO, dist_mgrid, ~, ~, ~] = compute_policy_and_distribution(money_supply, gamma, y, beta, sigma, theta_1, theta_2, QQ);
    
    % Calculate money demand (average money holdings)
    Dp = length(dist_mgrid);
    money_demand = (FO(2:Dp) - FO(1:Dp-1))' * ((dist_mgrid(2:Dp) + dist_mgrid(1:Dp-1))/2) + (FO(1) * dist_mgrid(1));
    
    % Compute excess demand
    excess = money_demand - money_supply;
end

% Function to compute policy functions and distributions given a money supply
function [g_pol_1, g_pol_2, FO, dist_mgrid, c_pol_1, c_pol_2, mgrid] = compute_policy_and_distribution(money_supply, gamma, y, beta, sigma, theta_1, theta_2, QQ)
    % Use the money supply to get tau
    tau = money_supply * gamma / (1 + gamma);
    ...
    % Maximum number of iterations for policy function
    Nk = 1000;
    ...
    for j = 1:Nk-1
        ...
        % Compute expectations under the Markov transition matrix
        EV_1 = ((beta/(1+gamma)) * (QQ(1,1) * uprime_1' + QQ(1,2) * uprime_2'))';
        EV_2 = ((beta/(1+gamma)) * (QQ(2,1) * uprime_1' + QQ(2,2) * uprime_2'))';
        ...
        gdiff = max(abs([g_1(:,j+1); g_2(:,j+1)] - [g_1(:,j); g_2(:,j)]));
        if gdiff <= 1e-10
            %disp(['Policy function convergence achieved after ', num2str(j), ' iterations.']);
            g_pol_1 = g_1(:, j+1);
            g_pol_2 = g_2(:, j+1);
            break;
        end
    ...
    % If convergence isn't achieved, store final iteration values
    if j == Nk-1
        %disp('Maximum iterations reached without convergence for policy functions.');
        g_pol_1 = g_1(:, end);
        g_pol_2 = g_2(:, end);
    end
    ...
    % Placeholders for distributions
    F1 = zeros(Dp, Dk);
    F1(:,1) = epr(1) * unif_dist;
    
    F2 = zeros(Dp, Dk);
    F2(:,1) = epr(2) * unif_dist;
    ...
    for i = 1:Dk-1
        ...
        if diff1 <= 1e-10 && diff2 <= 1e-10
            %disp(['Distribution convergence achieved after ', num2str(i), ' iterations.']);
            break;
        end
    end
    
    if i == Dk-1
        %disp('Maximum iterations reached without convergence for distributions.');
    end
    ...
    % Interpolate consumption to the distribution grid
    c_pol_1 = interp1(mgrid, c_pol_1, dist_mgrid, 'linear', 'extrap');
    c_pol_2 = interp1(mgrid, c_pol_2, dist_mgrid, 'linear', 'extrap');
end